 ...
 */
public class TypeDTateAffineNoDenomMillerPairingMap extends
		AbstractMillerPairingMap<Polynomial> {
	...
	public Element pairing(Point in1, Point in2) {
		...
		Polynomial Qx = (Polynomial) in2.getX().duplicate()
				.mul(pairing.nqrInverse);
		// v^-3/2 = v^-2 * v^1/2
		Polynomial Qy = (Polynomial) in2.getY().duplicate()
				.mul(pairing.nqrInverseSquare);

		return new GTFiniteElement(this, (GTFiniteField) pairing.getGT(),
				tatePow(pairing(in1, Qx, Qy)));
	}
	...
	public Element pairing(Element[] in1, Element[] in2) {
		...
		return new GTFiniteElement(this, (GTFiniteField) pairing.getGT(),
				tatePow(pairingInternal(in1, Qs)));
	}
	...
	public int getPairingPreProcessingLengthInBytes() {
		if (pairingPreProcessingLengthInBytes == -1) {
			pairingPreProcessingTableLength = pairing.r.bitLength()
					+ BigIntegerUtils.hammingWeight(pairing.r) - 1;
			pairingPreProcessingLengthInBytes = 4 + (pairingPreProcessingTableLength * 3 * pairing.Fq
					.getLengthInBytes());
		}
	...
	public boolean isAlmostCoddh(Element a, Element b, Element c, Element d) {
		...
		// v^-3/2 = v^-2 * v^1/2
		Element cy = ((Point) c).getY().duplicate()
				.mul(pairing.nqrInverseSquare);
		Element dy = ((Point) d).getY().duplicate()
				.mul(pairing.nqrInverseSquare);
	...
	final void qPower(int sign, PolyModElement e2, Element e0re, Element e0im,
			Element e0re0, Element e0im0, List<Element> inre, List<Element> inim) {
	...
	protected void millerStep(Point<Polynomial> out, Element a, Element b,
			Element c, Polynomial Qx, Polynomial Qy) {
		// a, b, c are in Fq
	...
	public class TypeDMillerNoDenomAffinePairingPreProcessing extends
			AbstractMillerPairingPreProcessing implements Serializable {
		...
		public Element pairing(Element in2) {
			...
			// map from twist: (x, y) --> (v^-1 x, v^-(3/2) y)
			// where v is the quadratic non-residue used to construct the twist
			Polynomial Qx = (Polynomial) pointIn2.getX().duplicate()
					.mul(pairing.nqrInverse);
			// v^-3/2 = v^-2 * v^1/2
			Polynomial Qy = (Polynomial) pointIn2.getY().duplicate()
					.mul(pairing.nqrInverseSquare);
			...
			for (int m = pairing.r.bitLength() - 2; m > 0; m--) {
				millerStep(f0, processingInfo.table[row][0],
						processingInfo.table[row][1],
						processingInfo.table[row][2], Qx, Qy);
				out.mul(f0);
				row++;
			...
			millerStep(f0, processingInfo.table[row][0],
					processingInfo.table[row][1], processingInfo.table[row][2],
					Qx, Qy);
			out.mul(f0);

			return new GTFiniteElement(
					TypeDTateAffineNoDenomMillerPairingMap.this,
					(GTFiniteField) pairing.getGT(), tatePow(out));
		}
	}
