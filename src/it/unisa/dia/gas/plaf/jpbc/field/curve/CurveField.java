 ...
 */
public class CurveField<F extends Field> extends
		AbstractFieldOver<F, CurveElement> implements Serializable {

	public static <F extends Field> CurveField<F> newCurveFieldJ(Random random,
			Element j, BigInteger order, BigInteger cofac) {
		// Assumes j != 0, 1728
	...
	public CurveField(Random random, Element a, Element b, BigInteger order,
			BigInteger cofac, byte[] genNoCofac) {
		super(random, (F) a.getField());
	...
	public CurveField(Random random, Element b, BigInteger order,
			BigInteger cofac) {
	...
	public boolean equals(Object o) {
		...
		if (!b.equals(that.b))
			return false;
		if (cofac != null ? !cofac.equals(that.cofac) : that.cofac != null)
			return false;
	...
	/**
	 * Existing points are invalidated as this mangles c. Consider the curve
	 * E�?given by Y^2 = X^3 + a v^2 X + v^3 b, which we call the (quadratic)
	 * twist of the curve E, where v is a quadratic nonresidue in Fq
	 * 
	...
	protected CurveElement getCurveRandomNoCofacSolvefory() {
		...
		do {
			t.set(element.getX().setToRandom()).square().add(a)
					.mul(element.getX()).add(b);
		} while (!t.isSqr());
	...
	public Element[] twice(Element[] elements) {
		...
		for (i = 0; i < n; i++) {
			...
			// e2=lambda = (3x^2 + a) / 2y
			e2.set(q.getX()).square().mul(3).add(a).mul(table[i]); // Recall
																	// that
																	// table[i]=1/2y_i
